...
class UserService {
  ...
  public createAccount(userName: string, email: string, password: string): Promise<{ user: User }> {
    return new Promise<{ user: User }>(async (resolve, reject) => {
      ...
      const refreshToken: string = jwt.sign({ userId: newUser.id }, process.env.SECRET!, {
        expiresIn: "7d",
      });
      const accessToken: string = jwt.sign({ userId: newUser.id }, process.env.SECRET!, {
        expiresIn: "1h",
      });
  ...
  public login(email: string, password: string) {
    return new Promise<{ user: User }>(async (resolve, reject) => {
      try {
        ...
        const refreshToken: string = jwt.sign({ userId: user.id }, process.env.SECRET!, {
          expiresIn: "7d",
        });
        const accessToken: string = jwt.sign({ userId: user.id }, process.env.SECRET!, {
          expiresIn: "1h",
        });
